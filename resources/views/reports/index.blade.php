            ...
            <div class="report-row row align-items-center">
                ...
                <div class="col">
                    <div class="form-group">
                        <label for="user_id">{{ __('app.reports_index_options_user_label') }}</label>
                        <div class="select">
                ...
                <div class="col-auto">
                    ...
                    <div class="dropdown d-inline">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <a href="{{ route('reports.user', ['user_id' => $current_user, 'month' => $now->format('n'), 'year' => $now->format('Y')]) }}" class="dropdown-item @if($active_item == 1) active @endif">{{ __('app.reports_index_options_period_thismonth_button') }}</a>
                            <a href="{{ route('reports.user', ['user_id' => $current_user, 'month' => $last_month->format('n'), 'year' => $last_month->format('Y')]) }}" class="dropdown-item @if($active_item == 2) active @endif">{{ __('app.reports_index_options_period_lastmonth_button') }}</a>
                            <a href="{{ route('reports.user', ['user_id' => $current_user, 'week' => $now->format('o\WW')]) }}" class="dropdown-item @if($active_item == 3) active @endif">{{ __('app.reports_index_options_period_thisweek_button') }}</a>
                            <a href="{{ route('reports.user', ['user_id' => $current_user, 'week' => $last_week->format('o\WW')]) }}" class="dropdown-item @if($active_item == 4) active @endif">{{ __('app.reports_index_options_period_lastweek_button') }}</a>
                            <div class="dropdown-divider"></div>
                            <a href="{{ route('reports.user', ['user_id' => $current_user, 'all' => 1]) }}" class="dropdown-item @if($active_item == 5) active @endif">{{ __('app.reports_index_options_period_showall_button') }}</a>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="row quick-stats">
    ...
    <div class="col">
        <div class="quick-stats__item bg-primary">
            <div class="quick-stats__info">
                <h2>{{ __('app.global_distance_format', ['distance' => (float) number_format($total_travel_distance, 1)]) }}</h2>
                <small>{{ __('app.reports_index_quickstats_traveldistance_label') }}</small>
            </div>
        </div>
    </div>
    ...
    <div class="col">
        <div class="quick-stats__item bg-primary">
            <div class="quick-stats__info">
                <h2>{{ __('app.global_money_format', ['rate' => number_format($total_travel_expense, 2)]) }}</h2>
                <small>{{ __('app.reports_index_quickstats_totaltravelexpenses_label') }}</small>
            </div>
        </div>
    </div>
    ...
    <div class="col">
        <div class="quick-stats__item bg-primary">
            <div class="quick-stats__info">
                <h2>{{ __('app.global_money_format', ['rate' => number_format($total_wage, 2)]) }}</h2>
                <small>{{ __('app.reports_index_quickstats_totalwage_label') }}</small>
            </div>
        </div>
    </div>
    @endcan
    ...
    <div class="card-body">
        ...
        <div class="tab-container">
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link" data-toggle="tab" href="#tab-worktype" role="tab">{{ __('app.reports_index_display_timebyworktype_table_title') }} ({{ count($entries_by_work_type) }})</a>
                </li>
            </ul>
            ...
            <div class="tab-content pb-0">
                ...
                <div class="tab-pane fade" id="tab-worktype" role="tabpanel">
                    <table class="table table-bordered mb-0">
                        <thead>
                            <tr>
                                <th>{{ __('app.reports_index_display_timebyworktype_table_worktype_column') }}</th>
                                <th>{{ __('app.reports_index_display_timebyworktype_table_time_column') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($entries_by_work_type as $key)
                            <tr>
                                <td>{{ $key['name'] }}</td>
                                <td>
                                    @if ($key['has_timer'])
                            ...
                            @empty
                            <tr>
                                <td colspan="2">{{ __('app.reports_index_display_timebyworktype_table_noentries_text') }}</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
